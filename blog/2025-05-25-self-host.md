...
It gave me a lot of fun to tinker with. I would like to share the thought
...
called [glitchtip](https://glitchtip.com/), and more.

The server with just 2 vCPUs and 4GB of RAM is able to power a lot of services.
The server itself cost less than $4 a month. Considering, Plausible cloud
service cost $9 a month, and most cloud providers' web services with much tinier
machine still cost a few dollars a month. Self-hosting is a great deal.
...
way to help anyone hesitate to start. This document is structured as if you were
following along self-hosting your own Playground server. I wish this document is
something I could read beforehand, telling me self-hosting is not that hard and
not that expensive!
...
own `ethanppl.com` domain. If you have a domain, you can skip forward.
...
Create an account in [Hetzner Cloud](https://hetzner.cloud/?ref=YMRI7xo3b2af).
There might be a few verifications steps involved to prevent abuse. Once the
account is active, you can rent a server.
...
- SSH keys: Create a pair and save the keys locally.
...
[SSH](https://en.wikipedia.org/wiki/Secure_Shell) into the machine. If you do
not know about SSH, you can think of it as connecting your computer to the
server computer with the terminal. It is like directly accessing the terminal of
the server over the Internet, securely.
...
[configuring DNS](#configuring-dns) section.
