...
Because we did not expose the Postgres container to the host network, we cannot
connect to it with `localhost:5432`. Instead, we run the command inside the
container with
[`docker exec`](https://docs.docker.com/reference/cli/docker/container/exec/):

```sh
docker exec -it postgres psql -U postgres -d playground_engine
```

If you are prompted to input your password. Input the same password set above in
the `POSTGRES_PASSWORD` ENV variable. If connected successfully, it means the
